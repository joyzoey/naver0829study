...
public class BoardService {
	...
	{
		int num=dto.getNum();//새글일경우 0,답글일경우0보다 큰 값이 들어있다
		...
		if(num==0) {
			...
			//답글인 겨우 그룹은 그대로 사용하므로 regroup은 따로 안구함
			//답글인 경우
			this.updateRestep(regroup, restep);//같은 그룹 중 전달받은 restep보다 큰 값이 있는 글들 모두 +1
			//그리고 나서 각각의 값에 +1
			restep=restep+1;
			relevel=relevel+1;
		}
